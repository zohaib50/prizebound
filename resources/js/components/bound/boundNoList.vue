        ...
        <table class="table table-striped w-100">
            ...
            <tr v-for="(item, index) in items">
                ...
                <td class="p-2 text-end">
                    <a :href="'/bound/list/'+item.id+'/edit'" class="me-2 btn btn-sm btn-outline-primary">
...
export default {
    ...
    methods:{
        getData(page= 1){
            axios.get('/bound/api/number?page='+page+'&search='+this.search+'&limit='+this.limit+'&category='+this.category+'&year='+this.year+'&postion='+this.postion).then(response => {
        ...
        deleteData(id){
            axios.delete('/bound/api/number/'+id).then(response =>{
                this.getData()
            })
        }
    }
